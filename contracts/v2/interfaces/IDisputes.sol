...
interface IDisputes is IV2Module {
    event DisputeOpened(uint256 indexed disputeId, uint256 indexed claimId, address indexed opener, bytes32 reasonHash, uint64 timestamp, uint16 version);
    event DisputeResolved(uint256 indexed disputeId, IV2Types.DisputeStatus status, address indexed resolver, uint64 timestamp, uint16 version);
