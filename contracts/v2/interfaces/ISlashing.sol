...
interface ISlashing is IV2Module {
    event SlashProposed(bytes32 indexed proposalId, uint256 indexed claimId, address indexed verifier, uint256 amount, bytes32 reason, uint64 timestamp, uint16 version);
    event SlashExecuted(bytes32 indexed proposalId, address indexed verifier, uint256 amount, uint64 timestamp, uint16 version);
