...
interface ITreasury is IV2Module {
    event FundsDeposited(address indexed from, uint256 amount, bytes32 indexed bucket, uint64 timestamp, uint16 version);
    event FundsWithdrawn(address indexed to, uint256 amount, bytes32 indexed bucket, uint64 timestamp, uint16 version);
