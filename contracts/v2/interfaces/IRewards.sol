...
interface IRewards is IV2Module {
    event RewardAccrued(uint256 indexed claimId, address indexed account, uint256 amount, uint64 timestamp, uint16 version);
    event RewardClaimed(address indexed account, uint256 amount, uint64 timestamp, uint16 version);
